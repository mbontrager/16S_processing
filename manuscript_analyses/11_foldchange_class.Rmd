...
knitr::opts_chunk$set(fig.width=12, fig.height=8, fig.path='Figs_foldchange_class/',
...
physeq_1 <- tax_glom(physeq, "Class")
...
if (output_table){
    ...
    write.csv(f, "output/sample_matrix_class.csv")
...
## Saltwater vs. Freshwater (Water only)
...
water_only <- subset_samples(physeq_1, Environment=="Water")

copdds <- phyloseq_to_deseq2(water_only, ~ WaterType)
geoMeans <- apply(counts(copdds), 1, gm_mean)
copdds <- estimateSizeFactors(copdds, geoMeans = geoMeans)
copdds <- DESeq(copdds, fitType = "local")
...
sigtab <- cbind(as(sigtab, "data.frame"), 
                as(tax_table(water_only)[rownames(sigtab), ], "matrix"))
sigtab <- sigtab[,1:9]
