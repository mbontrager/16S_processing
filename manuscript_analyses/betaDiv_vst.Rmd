...
plot_ordination(physeq, c.jaccard, color = "WaterType", 
                shape = "Environment") +
    ggtitle(paste("PCoA w/ jaccard Distance")) +
    geom_point(size = 6, alpha = 0.9) +
    scale_color_manual(values = getPalette(colorCount))
...
colorCount = length(levels(sample_data(ngnw)$Location))
#Jaccard distance
meas <- distance(ngnw, method = "jaccard")
# Ordinate
iMDS  <- ordinate(ngnw, "PCoA", distance=meas)

plot_ordination(ngnw, iMDS, color = "WaterType", 
                shape = "Environment") +
    ggtitle(paste("PCoA w/ jaccard Distance (Copepods Only)")) +
    geom_point(size = 6, alpha = 0.9) +
    scale_color_manual(values = getPalette(colorCount))
