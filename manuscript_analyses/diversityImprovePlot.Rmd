...
richness_plot(physeq, color_val, meas = c("Observed", "Chao1", "Simpson", "Shannon")) + geom_boxplot(data=alpha1$data, aes(x=Environment, y=value, color=NULL)) + 
...
richness_plot(physeq, color_val, col = "Environment", meas = c("Observed", "Chao1", "Simpson", "Shannon")) + 
    ...
    geom_point(size = 5, alpha = 1.0) + theme(axis.text.x = element_blank())
...
t.test(adivWsalt$Chao1, adivWfresh$Chao1, alternative="greater")
...
t.test(adivCsalt$Chao1, adivCfresh$Chao1, alternative="greater")
...
rplot_water <- richness_plot(water_only, color_val,xval = "WaterType", 
                             col = "WaterType", title = t, meas = c("Shannon")) #geom_point(shape=17, size=4)
