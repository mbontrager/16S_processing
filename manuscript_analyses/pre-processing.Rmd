...
sample_data(physeq)$RegionxWater <- paste(sample_data(physeq)$Region, 
                                          sample_data(physeq)$WaterType)
sample_data(physeq)$LatitudexWater <- paste(sample_data(physeq)$Latitude, 
                                          sample_data(physeq)$WaterType)
sample_data(physeq)$RegionxHost <- paste(sample_data(physeq)$Environment, 
                                          sample_data(physeq)$Region)
...
# Remove samples because they are either old (RAE), highly contaminated(MIE, TBE), redundant (V2E) or 
# inappropriate (V1W due to use of the 0.8 micron filter)
...
# Export csv count table if necessary
# I normalize everything percent of total per sample (better normalization is performed in during
...
if (output_table){
    ...
    f <- f[order(f$OTU),]
    write.csv(f, "output/sample_matrix_paired_relative_abundance.csv")
    rm(physeq1, b, c, d, e, f)
}

# Arrange data for easy parsing of shared presence/absence
b <- psmelt(physeq)[, 1:3]
c <- dcast(b, OTU ~ Sample)
e <- as.data.frame(tax_table(physeq))
e <- cbind(OTU = rownames(e), e)
...
d <- as.numeric(d)
f$OTU <- sprintf("OTU_%05d", d)
countTable <- f[order(f$OTU),]
rm(b, c, d, e, f)

# Print a table of read counts
...
# Print mean and median sample sums
